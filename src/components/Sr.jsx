  ...
  componentDidMount() {
    ...
    if(this.props.location.data!==undefined){
  		...
  		getTableRows({json:true,code:'wafyartvotes',scope:'wafyartvotes',table:'cates'}).then(data => {
    		var listaud=[];
                ...
                for (let i = 0; i < this.state.catelist.length; i++) {
                  getTableRows({json:true, code:'wafyartvotes', scope:this.state.catelist[i].catename ,table:'auditorlists',key_type:'i64',index_position:'2'}).then(data => {
  	                try{
  	...
  	handleVoteOk = (e) => {
	    ...
	    signfun('wafyartvotes','voteaud','self',parseInt(this.state.votenum*10000.0000,10),this.state.curcatename,this.state.curauditor);
	    this.setState({
	      voteable: false,
	    });
	}
          ...
          return (
            <Panel header={iter.catename} key={i} style={customPanelStyle}>
		        ...
		        >
       			 <h4>数目</h4>
       			 <Row><Col span={22}>
       			 <TextArea value={this.state.votenum} onChange={this.handleVoteNumChange} autosize />
       			 </Col><Col span={2}>&nbsp;MZP</Col></Row>
		        </Modal>
		        <Row>
		        <Col span={9}>
		        	<div >
					    <Card title={'分类：'+iter.catename} bordered={false} style={{ width: '80%' }}>
					      <p>{'详细信息：'+iter.memo}</p>
					      <p>{'创建者：'+iter.createname}</p>
					      <p>{'求助帖数目：'+iter.articlenum}</p>
					      <p>{'有效审核者：'+iter.auditornum+' 位'}</p>

					      <p>{'当前收益：'+iter.reword/10000.0000+' MZP'}</p>
					      <p>{'当前得票：'+iter.votetick/10000.0000+' MZP'}</p>
					      <p>{'最小支付金额：'+iter.paylimit/10000.0000+' MZP'}</p>
					      ...
					      <Row>
					      <Col span={12}>
					      	<Button type='primary' onClick={(ev)=>{this.regaudit(ev,iter.catename)}} ghost>申请审核者</Button>
					      </Col><Col span={12}>
					      	<Button type='primary' onClick={(ev)=>{this.logoutaudit(ev,iter.catename)}} ghost>退出审核者</Button>
					      </Col></Row>
					    </Card>
					  </div>
		        </Col>
		        ...
		        <h3>审核者列表(红色为当前有效审核者)</h3>
              	<List
	                ...
	                renderItem={item => (
					    <List.Item actions={[<Button type="primary" onClick={(ev)=>{this.showVoteModal(ev,item.auditor,iter.catename)}}>投票</Button>]}>
					    <List.Item.Meta 
					     	description={<div>
					      	<Row><Col span={10}><font color={this.contains(iter.catename,item.auditor)<iter.auditornum?'red':''}>
					      	{'账号：'+item.auditor}</font></Col>
					      	<Col span={10}><font color={this.contains(iter.catename,item.auditor)<iter.auditornum?'red':''}>{'得票数：'+item.amount/10000.0000+' MZP'}</font>
					      	</Col></Row>
					      	</div>}/>
					   </List.Item>
	                )}
