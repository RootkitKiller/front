...
class Wallet extends Component {
	...
	handleunstaOk = e => {
		...
		signfun(
			...
			'unstaketit',
			'self',
			parseInt(this.state.mzunstanum * 10000.0, 10)
		);
		this.setState({
			mzunstaable: false
		});
		...
		return (
			<Row>
				...
				>
					<h4>发送到</h4>
					<TextArea
						value={this.state.mztransto}
						onChange={this.handleMzToChange}
						autosize
					/>
					<h4>数目</h4>
					<Row>
						<Col span={22}>
							<TextArea
								value={this.state.mztransnum}
								onChange={this.handleMzNumChange}
								autosize
							/>
						</Col>
						<Col span={2}>&nbsp;MZ</Col>
					...
					<h4>备注</h4>
					<TextArea
						value={this.state.mztransmemo}
						onChange={this.handleMzMemoChange}
						autosize
					/>
				</Modal>
								...
								<List.Item.Meta
									title={
										<p>
											...
											&nbsp;&nbsp;
											{'时间：' +
												this.convertUTCTimeToLocalTime(item.block_time + 'Z')}
										</p>
									}
									description={
										<div>
